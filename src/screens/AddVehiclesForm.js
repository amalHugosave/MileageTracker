...
import RefuelFormInput from '../components/Inputs/RefuelFormInput';
import AddVehiclesFormInputs from '../components/Inputs/AddVehiclesFormInputs';
...
// console.log(sampleImages[0])
  ...
  const {addVehicleState, VehiclesArray} = useVehicleArrayStore();
  ...
  const [typeName , setTypeName] = useState('Vehicle Type')
data.type
...
const handlePress = ()=>{
  ...
  const handleFieldChange = (command , payload) =>{
    if(command == 'name'){
      ...
      setData((state)=>({...state , name : payload}));
      if(payload)
        setErrors({...errors , name : ''});
      else  
        setErrors({...errors , name : errorstext[0]})
    }else if(command == 'type'){
      setData((state)=>({...state , type : payload}));
      setTypeName(vehicleTypes[payload - 1])
      if(payload !== 0)
        setErrors({...errors , type : ''});
      else  
        setErrors({...errors , type : errorstext[1]})
    }else if(command == 'engine') {
      setData((state)=>({...state , engine : payload}))
    }else {
      // console.log("image change")
      setData((stat)=>({...data , image : payload}));
    }
  }
  ...
  const handleSubmit =  ()=>{
    ...
    if(VehiclesArray.length == 0){
      setVehicle({...data,  image , userId : id , vehId : newId});
    ...
    addVehicleState({_id : newId,
      ...
      image : image,
      userId : id})
      setData({name : '',type :0 ,engine : '' ,image : '' })
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.inputContainer}>
            <AddVehiclesFormInputs placeholder='Vehicle Name' id='name' handleData={handleFieldChange} val={data.name} Style={styles.input}/>
            {/* <TextInput value={data.name} onChangeText={(text)=>handleFieldChange('name' , text)} style={styles.input} placeholder='Vehicle Name'/> */}
            {errors.name && <Text style={styles.error}>{errors.name}</Text>}
           <Picker name={typeName} list={[
                { name: '2 Wheeler', _id: '2' },
                { name: '3 Wheeler', _id: '3' },
                { name: '4 Wheeler', _id: '4' },
                { name: 'other', _id: '5' },
            ]} handleSelectChange={handleFieldChange} conStyles={300} type='type' Styles={styles.Styles}/>
            
            {errors.type && <Text style={styles.error}>{errors.type}</Text>}
            <AddVehiclesFormInputs placeholder='Engine CC' id='engine' handleData={handleFieldChange} val={data.engine} Style={styles.input}/>

            {/* <TextInput value={data.engine} onChangeText={(text)=>handleFieldChange('engine' , text)} style={styles.input} placeholder='Engine CC'/> */}
            {errors.engine && <Text style={styles.error}>{errors.engine}</Text>}
        </View>
        <View style={styles.bottom}>
            <DoubleButton textHollow ="Cancel" handlehollowPress={()=>{setData({name : '',type :0 ,engine : '' ,image : '' }) ;navigation.navigate('vehiclesInfo')}} handleSolidPress={handleSubmit}
